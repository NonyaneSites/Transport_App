...
import {
  ...
  Check, Clock, StickyNote, Sparkles, ArrowUpDown
} from 'lucide-react';
...
export function VehicleAllocation({ manifest, serviceLabel, service, onSave }: Props) {
  ...
  // chronological order in which the Admin assigned each stop/hub
  // (vehicle.orderedStops) — never sorted alphabetically or by count.
  function ridersGroupedByHub(vehicle: Vehicle): { label: string; riders: Passenger[] }[] {
    ...
    // Any group not covered by orderedStops (legacy data) is appended after,
    // sorted by count so nothing silently disappears.
   ...
   * - Keeps passengers from the same physical sub-stop together unless group exceeds capacity.
   * - Appends stop/hub to vehicle.orderedStops maintaining route sequencing order.
   */
  ...
  /** Shifts the sequence of orderedStops up or down for custom route tuning */
  ...
  async function unassignRider(vehicleId: string, passengerId: string) {
    ...
    const updatedVehicles = manifest.vehicles.map((v) => {
      ...
      // Clean up orderedStops if no riders remain from that stop/hub
      ...
      return { ...v, riders: nextRiders, orderedStops: nextOrderedStops };
    });
    await onSave({ ...manifest, signups: updatedSignups, vehicles: updatedVehicles });
  }
      ...
      {manifest.vehicles.length === 0 ? (
        ...
        <div className="space-y-3">
          {sortedVehicles.map((vehicle) => {
            ...
            const detectedOfficialRep = detectVehicleRep(riders);
            const repStruct = vehicle.repName ? getRepStructure(vehicle.repName) : (detectedOfficialRep ? getRepStructure(detectedOfficialRep) : null);
            const groups = ridersGroupedByHub(vehicle);
              ...
              >
                  ...
                  >
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {detectedOfficialRep && !vehicle.repName && (
                          <span className="badge bg-crimson-500/15 text-crimson-300 text-[10px] flex items-center gap-1">
                            ...
                            Official Rep on board: {detectedOfficialRep}
                          </span>
                        )}
                      </div>
                ...
                {isExpanded && (
                  <div className="border-t border-line bg-bg/40 p-4 animate-fade-in">
                    {/* Rep assignment & Smart Auto-Suggestion */}
                    <div className="mb-4 space-y-2">
                      ...
                      {detectedOfficialRep && vehicle.repName !== detectedOfficialRep && (
                        <div className="flex items-center justify-between gap-2 rounded-lg border border-crimson-500/30 bg-crimson-500/10 px-3 py-2 text-xs">
                          <span className="flex items-center gap-1.5 text-crimson-300">
                            <Sparkles className="h-3.5 w-3.5 text-crimson-400" />
                            Official Structure Rep detected among riders: <strong className="text-ink">{detectedOfficialRep}</strong> {repStruct ? `(${repStruct})` : ''}
                          </span>
                          <button
                            type="button"
                            onClick={() => setRepName(vehicle.id, detectedOfficialRep)}
                            className="btn-crimson py-1 px-2.5 text-[11px] whitespace-nowrap"
                          >
                            Auto-Assign Rep
                          </button>
                        </div>
                            ...
                            <div key={group.label} className="rounded-xl border border-line bg-card/60 p-3">
                              <div className="mb-2 flex flex-wrap items-center gap-2">
                                <div className="flex items-center gap-1">
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => wrap(() => moveStopOrder(vehicle.id, group.label, 'up'))}
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => wrap(() => moveStopOrder(vehicle.id, group.label, 'down'))}
                                      ...
                                      <div className="flex items-center gap-2 flex-wrap">
                                        <span className={`text-sm ${p.present ? 'text-success-light' : 'text-ink'}`}>
                                          ...
                                          {vehicle.repName && p.fullName === vehicle.repName && (
                                            <span className="ml-1 font-bold text-crimson-400">* (Assigned Rep)</span>
                                          )}
                                        </span>
                                        {p.structure && (
